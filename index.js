...
app.post('/webhook', async (req, res) => {
  ...
  // ✅ Ignore empty messages (delivery callbacks etc)
  if (!body || !from) {
    res.sendStatus(200);
    return;
  }
  ...
  // ✅ Filter out any corrupted empty entries
  conversations[from] = conversations[from].filter(m => m.content && m.content.trim() !== '');
